...
void MGHImageIO::ReadVolumeHeader(gzFile fp) {
  ...
  if (RASgood) {
    ...
    */
    ...
    for (unsigned int ui = 0; ui < 3; ++ui) TReadZ(fp, c[ui]);
...
#else
      for (unsigned int uj = 0; uj < 3; ++uj) vDir.push_back(matrix[uj][ui]);
#endif
...
void MGHImageIO::WriteImageInformation() {
  ...
  if (extension == std::string("mgh")) {
    ...
    if (!fp) itkExceptionMacro(<< " Failed to open gzFile for writing");
    GZWrapper writer(fp);
    this->WriteHeader(writer);
    gzclose(fp);
  }
...
void MGHImageIO::Write(const void* buffer) {
  ...
  if (extension == std::string("mgh")) {
    ...
    if (!file_p) itkExceptionMacro(<< " Failed to open gzFile for writing");
