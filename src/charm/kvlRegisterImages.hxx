...
RegisterImages<TransformationType, MetricType>::RegisterImages() {
  m_FixedImage = 0;
  m_MovingImage = 0;
...
template <typename TransformationType, typename MetricType>
void RegisterImages<TransformationType, MetricType>::ReadImages(
    const char* fileNameT1, const char* fileNameT2) {
...
template <typename TransformationType, typename MetricType>
void RegisterImages<TransformationType, MetricType>::InitializeTransform() {
  typedef itk::CenteredTransformInitializer<TransformationType, ImageType,
                                            ImageType>
  ...
  typename TransformationType::MatrixType initialMatrix =
      m_InitialTransform->GetMatrix();
  typename TransformationType::OffsetType initialOffset =
      m_InitialTransform->GetOffset();
  std::cout << "Initial matrix = " << std::endl << initialMatrix << std::endl;
...
template <typename TransformationType, typename MetricType>
void RegisterImages<TransformationType, MetricType>::RunRegistration() {
  ...
  std::cout << "   Moving image: "
            << registration->GetInitialTransform()->GetParameters()
            << std::endl;
  std::cout << "   Fixed image:  "
            << registration->GetInitialTransform()->GetFixedParameters() << "\n"
            << std::endl;
  ...
  OptimizerScalesType optimizerScales(
      m_InitialTransform->GetNumberOfParameters());
  ...
  for (auto i = 0; i < m_ShrinkScales.size(); ++i) {
    shrinkFactorsPerLevel[i] =
        static_cast<unsigned long>(pow(2, m_ShrinkScales.at(i)));
  }
  ...
  try {
    ...
    if (((stopCondition.find("Step too small") != std::string::npos) ||
         (stopCondition.find("Convergence checker passed at iteration") !=
          std::string::npos)) == false) {
      std::cout << "Warning message:" << std::endl;
      std::cout << "   Model failed to converge!" << std::endl;

      if (stopCondition.find("Maximum number of iterations") !=
          std::string::npos) {
        std::cout << "   Try to increase the number of iterations by adding "
                     "the option \"-i 500\" by the end of the command"
                  << std::endl;
      }
    }
...
template <typename TransformationType, typename MetricType>
void RegisterImages<TransformationType, MetricType>::WriteOutputImage(
    std::string outImageFile, ImageType::Pointer resampledImage) {
...
template <typename TransformationType, typename MetricType>
void RegisterImages<TransformationType, MetricType>::WriteOutResults(
  ...
  if (m_Interpolator == std::string("l")) {
    ...
    std::cout << "The BSpline order: " << bsplineInterpolator->GetSplineOrder()
              << std::endl;
