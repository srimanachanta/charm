...
class RegistrationInterfaceCommand : public itk::Command {
 ...
 public:
  ...
  void Execute(itk::Object* object, const itk::EventObject& event) override {
    ...
    // // Check the values of the shrinking factors. If any of them is larger
    // than the size of the fixed image, update to the size of the fixed image
    // auto
    // sizeOfFixedImage=registration->GetFixedImage()->GetLargestPossibleRegion().GetSize();
    // Note: I tried to add the 2-D option in the program, but the class
    // itk::ImageRegistrationMethodv4 implicitly calls the filter
    // RecursiveGaussianImageFilter This filter requires a minimum of
    // 2(numberOfLevels -1) pixels along the dimension to be processed. for
    // (auto i=0; i < shrinkFactors.Length; ++i)
    // {
    //   shrinkFactors[i] = ((shrinkFactors[i] < sizeOfFixedImage[i]) ?
    //   shrinkFactors[i] : sizeOfFixedImage[i]);
    // }
    // registration->SetShrinkFactorsPerDimension(currentLevel, shrinkFactors);
    //
    // std::cout << registration->GetShrinkFactorsPerDimension(currentLevel) <<
    // std::endl;
    ...
    // factor of 5 in order to allow the optimizer to focus on progressively
    // smaller regions. The learning rate is set up to the current step length.
    // In this way, when the optimizer is reinitialized at the beginning of the
    // registration process for the next level, the step length will simply
    // start with the last value used for the previous level. This will
    // guarantee the continuity of the path taken by the optimizer through the
    // parameter space.
    /*
  ...
  // Define the version of the \code{Execute()} method accepting the input
  // object
...
class RegisterImages : public itk::Object {
 public:
  ...
  void WriteOutputImage(std::string outImageFile,
  ...
  typename MetricType::Pointer GetMetric() { return m_Metric; }

  void SetTranslationScale(double transScale) {
    m_TranslationScale = transScale;
  }

  void SetNumberOfIterations(int numIter) { m_NumberOfIterations = numIter; }
